...
class SignupController extends GetxController {
  ...
  String _hardOfHearing = '';
  String get hearingType =>_hardOfHearing;

  void setHearingType(String type) {
    _hardOfHearing = type;
    print("Are you hard of hearing? $_hardOfHearing");
    update();
  }
    ...
    if (formKey.currentState!.validate()) {
      try {
        ...
        Map<String, String> data = {
          ...
          'hardofhearing': _hardOfHearing
        };

        bool? usernameExists = await isUsernameExists();
        bool? emailExists = await isEmailExists();

        if (usernameExists != null && emailExists != null ) {
          if (!usernameExists && !emailExists) {
            http.Response response = await httpService.post('/user/register', data);
            ...
            loading(false);
            showSnackbar(SnackbarMessage.error, 'Email already exists');
          } else {
            loading(false);
  ...
  Future<bool?> isUsernameExists() async {
    http.Response response = await httpService.get('/user/checkusername/${usernameController.text.trim()}');
  ...
  Future<bool?> isEmailExists() async {
    http.Response response = await httpService.get('/user/checkemail/${emailController.text.trim()}');
    var data = json.decode(response.body);
    if (response.statusCode == 200 || response.statusCode == 201) {
      bool isExists = data['status'];
      return isExists;
